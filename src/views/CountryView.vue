<template>
    <div class="container country-details p-5">
        ...
        <div class="row country-details__body" v-if="countryDetails[0] && countryDetails[0].flags">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 col-sm-12 details__item details__item--margin">
                        <p>
                            ...
                            <span v-for="(domain, index) in countryDetails[0].topLevelDomain" :key="index"> {{ domain }}</span>
                        </p>
                        <p>
                            <span class="item__title">
                                Currencies:
                            </span>
                            <span v-for="(currency, index) in countryDetails[0].currencies" :key="index"> {{ currency.name}} </span>
                        </p>
                        <p>
                            <span class="item__title">
                                Languages:
                            </span>
                            <span v-for="(language, index) in countryDetails[0].languages" :key="index"> {{ language.name }}</span>
                        </p>
                    </div>
                    <div class="col-12 details__item details__item--border mt-5">
                        ...
                        <span v-for="(border, index) in countryDetails[0].borders" :key="index" class="item__border">
...
export default {
    ...
    methods:{
        ...
        async getAllCountriesList() {
            await this.getCountriesList({'type': 'name', 'value':this.$route.params.name})
            this.countryDetails = this.getAllCountries
            const test = this.countryDetails.find(country => country.name === this.$route.params.name || country.alpha3Code === this.$route.params.name)
            console.log(test)
        }
    }
}
